...
| GeoMap projections (albersUsa, mercator, equalEarth, identity) | `packages/engine/src/geo/projections.ts` → `createProjection()`. Default inference lives here too: `resolveDefaultProjection(topology)` (pre-projected bbox → identity + `PROJECTION_INFERRED` warning, lon span > 200 → equalEarth, else albersUsa). `normalize.ts` leaves `geo.projection` undefined; `compileGeoMap` resolves it. |
...
| Graph surface (canvas background, node knockout ring) | `packages/engine/src/graphs/encoding.ts` → `resolveGraphSurface(theme)`, mirrored by `vanilla/src/theme-tokens.ts` → `resolvedSurface(theme)` which `graph-mount.ts` stamps as `--oc-bg` on the wrapper. Node strokes are a 1.5px ring in that color. `graph.css` carries NO dark-mode canvas override (the old `#0d1117` is gone); `e2e/invariants/graph-surface.spec.ts` locks it. |
