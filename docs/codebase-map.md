...
| `ThemeColors.categoricalFill` (quieter fill variant for bar/area/arc/waffle/calendar/rect; copied from `categorical` when unset) and `hairline` (structural line, distinct from `gridline`) | `types/theme.ts`, `theme/defaults.ts`, `theme/resolve.ts` (`themeConfigToPartial`); picked per mark type by `packages/engine/src/compile/color-scale-range.ts` (`FILL_MARKS` set, `categoricalColorsForDomain`) |
| Derived neutral gray ramp (`deriveNeutralRamp`, on `ResolvedTheme.colors.neutral`) | `packages/core/src/colors/neutral.ts`. Also carries `neutral.surface`: the one answer to "what opaque color does this theme paint on" (the background when it is opaque, else the mode's `--oc-bg` token). Knockout rings (`engine/charts/utils.ts` → `resolveKnockoutColor`), table fills (`engine/tables/utils.ts` → `resolveTableSurface`), graph rings (`engine/graphs/encoding.ts` → `resolveGraphSurface`), tilemap label blending and `vanilla/theme-tokens.ts` → `resolvedSurface` all read it — no `--oc-bg` resolution outside core. |
...
| Animation timing parity guard (engine `ResolvedAnimation` defaults vs `--oc-animation-*` CSS tokens vs the canvas entrance budget) | `packages/engine/src/compiler/__tests__/animation-token-parity.test.ts` |
...
| Annotations: chrome guard (labels can't climb into the title block) | `packages/engine/src/annotations/collisions.ts` → `clampAnnotationsToBounds(annotations, svgWidth, svgHeight, measure, topBound)`. `topBound` is the first y a label may occupy; `compile.ts` passes `dims.topPad + dims.chrome.topHeight` (`LayoutDimensions.topPad` in `engine/layout/dimensions.ts`) when there is a chrome block, and the same band is fed to auto-placement as a `'chrome'` `ObstacleKind` (`annotations/placement.ts`). |
...
| Size legend (graduated circles + title row) | `packages/engine/src/legend/size.ts` (`computeSizeLegendContent`, `placeSizeLegend`, `sizeLegendTitleStyle`). The title comes from `size.title ?? size.field`; the map's own copy is built inline in `geo/compile-geo-map.ts` (`pointSizeLegend`). Rendered by `renderSizeLegend` in `packages/vanilla/src/renderers/legend.ts`. |
| Knockout ring / seam color (scatter, line points, dot, beeswarm, pie seams, stacked seams, map bubbles) | `packages/engine/src/charts/utils.ts` → `resolveKnockoutColor(theme)`. Resolved theme background when opaque, else the mode surface. `stackSeamStroke()` delegates to it. Never hardcode `'#ffffff'` — the `terminal` preset is dark in both modes. |
...
| Renderer: marks (line, area, rect, arc, point, text, rule, tick) | `packages/vanilla/src/renderers/marks.ts` → `renderMarks()`. Returns an optional `<g class="oc-mark-labels">` overlay that `svg-renderer.ts` appends outside the clip path so value labels on tall columns aren't clipped — `.oc-annotations` is appended *before* that overlay, so annotations paint under the data labels. Label halos: `applyLabelHalo` (skipped when the engine set `halo: false`) delegates to the one `applyKnockoutHalo(el, color, fontSize)` in `renderers/svg-dom.ts`, shared with the annotation labels; presentation attributes, stroke width `round(fontSize * 0.3)`. |
| Renderer: axes | `packages/vanilla/src/renderers/axes.ts` → `renderAxes()` |
| Renderer: annotations | `packages/vanilla/src/renderers/annotations.ts` → `renderAnnotations()`. Knockout halo behind label text comes from `applyKnockoutHalo` in `renderers/svg-dom.ts` (same helper the mark labels use). |
...
| Legend click-toggle + hover-link wiring (real `role="button"` entries, `aria-pressed`, 24px hit rect) | `packages/vanilla/src/interactions/legend-interaction.ts` → `wireLegendInteraction`. |
| Keyboard nav for line/area charts (arrow keys drive the crosshair when no per-mark tooltip descriptors exist) | `packages/vanilla/src/interactions/keyboard-nav.ts` → `wireKeyboardNav`; gates on `layout.tooltipDescriptors.size === 0`, driving the `CrosshairController` from `interactions/crosshair.ts`. |
...
| Table density / cards mode / delta chips / totals | Engine: `packages/engine/src/tables/compile-table.ts` (`determineCellType`, `inferColumnType`, `inferAlignment`, `resolveSort`, `buildTotalRow`), `tables/utils.ts` (`resolveTableSurface`, `accessibleTextColor`), `tables/sparkline.ts` (`computeSparklineDomain`). Spec surface: `TableSpec.density/striped/sort/totalRow` (`core/src/types/spec.ts`), `ColumnConfig.type/delta/priority/total` (`core/src/types/table.ts`; `total: false` blanks a column's total cell). Widths: `resolveColumns`/`estimateColumnWidth` in `compile-table.ts` — an explicit `width` is fixed (never flex-scaled up), percent widths are rounded, overflow scales the explicit widths down against `MIN_COLUMN_WIDTH` (60) and warns `TABLE_WIDTH_OVERFLOW`; `cellPaddingX(density)` (24/32/40 for condensed/regular/relaxed) must track the `th, td` padding in `table.css`, which is `box-sizing: border-box`. DOM: `packages/vanilla/src/table-renderer.ts` (`renderTfoot`, `data-label`/`data-priority`), `renderers/table-cells.ts` (`renderDeltaCell`), `table-mount.ts` (`applyBreakpointClass`: explicit density wins, else content-pressure condense, cards below 400px). CSS `packages/core/src/styles/table.css`. |
...
| Rendered layout invariants (real Chromium `getBoundingClientRect`, catches engine-vs-renderer disagreements happy-dom can't) | `e2e/invariants/rendered-invariants.spec.ts` (reuses the visual-suite story slugs); table column widths: `e2e/invariants/table-column-width.spec.ts` (an explicit `200px` column renders at exactly 200px, no horizontal overflow). Others in `e2e/invariants/`: `canvas-alpha-parity`, `font-race`, `graph-surface`, `known-bugs`. |
...
| Frozen e2e fixture copies (pinned visual/invariant stories, pixel-identical, `Testing / Fixtures`) + frozen stylesheet | `examples/src/testing/fixtures-*.stories.tsx` (+ `testing.css`, the `.tfix-` frozen namespace). Slugs are `testing--fixtures--*`. Includes `rotated-with-source`, moved here from `examples/src/charts/`. Design-refresh fixtures live in `fixtures-refresh.stories.tsx` (flat columns, donut `centerLabel`, presets), `fixtures-tables.stories.tsx` (density, delta chips, heatmap dark, shared sparklines, cards), `fixtures-tilemaps.stories.tsx`, `fixtures-sankey.stories.tsx` (energy light/dark, `other` bucket), `fixtures-dashboards.stories.tsx` (KPI tiles, 140px tiny tile); `us-bubbles` and `world-diverging-quantize` are appended to `fixtures-maps.stories.tsx`. |
...
| GeoMap compile (projection, data join, choropleth marks, point marks, legends, focus) | `packages/engine/src/geo/compile-geo-map.ts` → `compileGeoMap()`. Helpers: `buildBasemapMarks`, `buildQuantitativeMarks`, `buildCategoricalMarks`, `resolveFocus`. Unjoined features get `noData: true` + `pattern` on `GeoMapFeatureMark` — only under a color encoding (a basemap has nothing to be missing). Both legends carry a detached `noData` swatch keying that hatch, gated on `marks.some(m => m.noData)`. Rendering: `buildPatternDefs`/`resolvePatternFill` in `vanilla/src/map-renderer.ts` emit one shared `<pattern>` def and resolve the swatch fill to its `url(#id)`; the swatch itself is drawn by the shared `renderNoDataSwatch` in `renderers/legend.ts` (the categorical branch passes a rounded chip + `dominant-baseline: central` so it matches its entries). The categorical map legend emits its own `entryPositions` via `wrapCategoricalEntries`, mirroring the renderer's wrap rule, so the "No data" key lands after the last entry (or on a row of its own) instead of off the frame. |
| GeoMap data join (feature id matching) | `packages/engine/src/geo/join.ts` → `joinDataToFeatures()` |
| GeoMap projections (albersUsa, mercator, equalEarth, identity) | `packages/engine/src/geo/projections.ts` → `createProjection()`. Default inference lives here too: `resolveDefaultProjection(topology)` (pre-projected bbox → identity + a dev-only `PROJECTION_INFERRED` warning, lon span > 200 → equalEarth, else albersUsa). `normalize.ts` leaves `geo.projection` undefined; `compileGeoMap` resolves it. |
| Map/legend class scale (one scale shared by fills and swatches) | `packages/engine/src/legend/continuous.ts` → `buildClassScale()` / `classScaleForChannel()`. Default is quantize with `tickStep` round breaks; quantile/threshold only when authored; a diverging ramp over a zero-straddling domain forces an odd, zero-centered classing. |
| Compact `Less ▢▢▢▢▢ More` class key (calendar) | `packages/engine/src/legend/continuous.ts` → `computeCompactClassLegendContent()`. Selected in `legend/compute.ts` for `markType === 'calendar'` when the spec sets no `legend.position`; placed bottom-right via `ContinuousLegendContent.align: 'right'` + `barOffsetX` + `labelBaselineOffset` (labels sit *on* the swatch row, not under it). |
| Chrome economy (what a small container may draw) | `packages/core/src/responsive/breakpoints.ts` → `resolveChromeEconomy(width, height)`, `getHeightClass(height)` (`HeightClass` `'minimal'` under `HEIGHT_MINIMAL_MAX` 100 → chrome hidden, `'cramped'` under `HEIGHT_CRAMPED_MAX` 200 → compact chrome), `HEIGHT_NO_GRID_MAX` 150, `COMPACT_MAX_X_TICKS` 3. In the cramped band the default title shrinks by `CRAMPED_TITLE_SCALE` (`core/src/layout/chrome.ts`, `computeChrome`'s `crampedTitle` param, set in `engine/layout/plan.ts`) and the metric bar / series-search bands are gated off (`engine/layout/dimensions.ts`, `isCrampedHeight`) — both would otherwise blow the min-chart-height guardrail and strip the title back out. Consumed once, in `compile.ts` (feeds `skipX`/`skipY` and `AxesDataContext.suppressGrid` / `.maxXTicks`). Explicit `axis` / `axis.grid` always wins. Facet panels are deliberately NOT run through it (the economy is a container-size rule, not a panel-size rule). |
| Sparkline data-arrival pulse | `packages/vanilla/src/transition/driver.ts` → `pulseSparklineTerminator()` in `finish()`; keyframe `oc-pulse` in `core/src/styles/keyframes.css`, rule + reduced-motion opt-out in `sparkline.css`. Only fires when the SVG carries `data-display="sparkline"`. |
| Waffle "Other" bucketing (opt-in) | `mark.other: number | { threshold, label? }`. Implemented in `packages/engine/src/compiler/normalize.ts` → `applyWaffleOther()` — it rewrites the *rows*, not the marks, so the grid, the color scale and the legend cannot disagree. Emits a `WAFFLE_OTHER_BUCKET` warning. Same spelling/shape as `SankeySpec.other`. |
| Calendar color-scale default | `packages/engine/src/compiler/normalize.ts` → `withCalendarColorScale()` defaults `encoding.color.scale.type` to `'quantize'` (5 classes via `DEFAULT_BIN_COUNT`). A recorded zero therefore lands on the lightest class and stays distinct from a missing day (`theme.colors.annotationFill`). |
| Dashboard gallery tiles (Panel / TileTitle / StatCard / DeltaChip / SparklineCard) | `examples/src/gallery/dashboards.layouts.tsx`. `dashTokens()` returns `--oc-*` var strings only; `useDashRootClass()` stamps `oc-root` (+ `oc-dark`) on each dashboard root so the token cascade reaches the HTML tiles. No local hex palette. |
...
| GeoMap mount (lifecycle, events, tooltips, resize, export) | `packages/vanilla/src/map-mount.ts` → `createGeoMap()`. Event type: `GeoMapMarkEvent` (discriminated by `kind: 'feature' \| 'point'`). Hover: feature = `oc-map-feature--hover` + raise (`raiseFeature`/`restoreFeatureOrder`, no sibling dim); legend swatch = `data-hover` on `.oc-map-features` matched against each feature's `data-bin-index`. Rules in `core/src/styles/interaction.css`. |
...
| Donut center stat (opt-in `markDef.centerLabel: string \| { text; subtitle? }`) | `packages/engine/src/charts/pie/index.ts` (`centerLabelMarks`, arc-only) emits two decorative `textMark`s (value at `metricValue`/600, subtitle at `small`/400) on top of the donut hole. |
...
| Graph canvas renderer (batched draw passes, focus tiers, label budget) | `packages/vanilla/src/graph/canvas-renderer.ts` → `GraphCanvasRenderer`. `labelBudget(zoom)` = `clamp(round(zoom*30), 12, 80)` non-forced labels, placed greedily by priority with an AABB declutter; hovered/selected/`alwaysShowLabel`/search-matched labels bypass both and are the only ones with a halo. Resting edge alpha is 0.3 light / 0.25 dark; the dim tier is `interaction.dimOpacity` (0.3) for nodes and a third of it for edges. |
| Graph surface (canvas background, node knockout ring) | `packages/engine/src/graphs/encoding.ts` → `resolveGraphSurface(theme)` and `vanilla/src/theme-tokens.ts` → `resolvedSurface(theme)`, both thin reads of `theme.colors.neutral.surface` which `graph-mount.ts` stamps as `--oc-bg` on the wrapper. Node strokes are a 1.5px ring in that color. `graph.css` carries NO dark-mode canvas override (the old `#0d1117` is gone); `e2e/invariants/graph-surface.spec.ts` locks it. |
...
| Canvas entrance animation | `packages/vanilla/src/scatter-canvas/entrance.ts` → `playCanvasEntrance()`, `computeEntranceDuration()`, `clampStagger()`. Replicates the CSS point entrance (fade-only, 40% of duration, 300ms total stagger budget, mirroring `MAX_TOTAL_STAGGER_MS` in `engine/src/compiler/animation.ts`). Its `totalMs` MUST be passed to `setupAnimationCleanup` -- the DOM-counting estimate sees no point elements in canvas mode. |
...
| Sankey compile (layout, label gutters, "Other" bucketing, links) | `packages/engine/src/sankey/compile-sankey.ts` → `compileSankey()`; d3-sankey wrapper in `sankey/layout.ts`. Labels: first column outside-left, last outside-right, interior right, with a symmetric gutter reservation capped at 35% of the width (`MAX_LABEL_GUTTER`). Each node carries a `valueLabel` tspan (tabular, axis ink) — dropped on interior columns when it would run into the next column. `SankeySpec.other` (opt-in) buckets sub-threshold nodes per column into a neutral "Other" node, preserving total flow. |
| Sankey render + path hover | `packages/vanilla/src/sankey-renderer.ts` (`renderLabels` stamps `.oc-sankey-label[data-node-id]` and the value tspan), `sankey-mount.ts` → `tracePath()` / `highlightPath()` (BFS upstream + downstream; on-path links 0.7, off-path 0.12, off-path nodes and labels 0.3). Transitions live in `packages/core/src/styles/sankey.css`. |
...
- The engine resolves `AnimationSpec` → `ResolvedAnimation` (`engine/src/compiler/animation.ts`). Defaults: enter 450ms/30ms stagger, update 450ms, exit 250ms, annotation delay 150ms, `MAX_TOTAL_STAGGER_MS` 300 (mirrored in `vanilla/src/scatter-canvas/entrance.ts` for the canvas point layer, which cannot import the engine at runtime).
