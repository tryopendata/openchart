...
| Dashboard gallery stories | `examples/src/gallery/dashboards.stories.tsx` + demo index `dashboards.demos.ts`. The four composed dashboard layouts (SaaS analytics, ops/monitoring, finance/markets, marketing funnel) plus shared tile primitives (`Panel`, `StatCard`, `TileTitle`) and the `dashTokens`/`pctChange`/`sparklineSpec` helpers live in `examples/src/gallery/dashboards.layouts.tsx`, a non-story module — Ladle turns every named export in a `*.stories.tsx` file into a story, so the layout components are split out to avoid that. |
