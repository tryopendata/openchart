...
| Annotations: shared constants (`ANCHOR_OFFSET=8`, `CONNECTOR_STANDOFF=6`, `MIN_CONNECTOR_LENGTH=14`, dot defaults, `LEDE_FONT_WEIGHT`, connector strokes, default font) | `packages/engine/src/annotations/constants.ts` |
...
- **Connector suppression:** a connector shorter than `MIN_CONNECTOR_LENGTH` (14) after pullback, or one whose target is inside the text block, is dropped. The dot survives. Most plain `{type:'text'}` annotations land here, which is why they show a marker and no leader.
...
- **Alignment:** nothing center-aligns. `textAnchor` is `'end'` when `anchor: 'left'` (block sits left of the point, right edge faces it) and `'start'` otherwise; the auto-placement `DIRECTIONS` table in `placement.ts` assigns start/end by direction.
