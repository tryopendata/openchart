...
| Derived neutral gray ramp (`deriveNeutralRamp`, on `ResolvedTheme.colors.neutral`) | `packages/core/src/colors/neutral.ts`. Also carries `neutral.surface`: the one answer to "what opaque color does this theme paint on" (the background when it is opaque, else the mode's `--oc-bg` token). Knockout rings (`engine/charts/utils.ts` → `resolveKnockoutColor`), table fills (`engine/tables/utils.ts` → `resolveTableSurface`), graph rings (`engine/graphs/encoding.ts` → `resolveGraphSurface`), tilemap label blending and `vanilla/theme-tokens.ts` → `resolvedSurface` all read it — no `cssTokenDefault` calls outside core. |
...
| Table density / cards mode / delta chips / totals | Engine: `packages/engine/src/tables/compile-table.ts` (`determineCellType`, `inferColumnType`, `inferAlignment`, `resolveSort`, `buildTotalRow`), `tables/utils.ts` (`resolveTableSurface`, `accessibleTextColor`), `tables/sparkline.ts` (`computeSparklineDomain`). Spec surface: `TableSpec.density/striped/sort/totalRow` (`core/src/types/spec.ts`), `ColumnConfig.type/delta/priority` (`core/src/types/table.ts`). DOM: `packages/vanilla/src/table-renderer.ts` (`renderTfoot`, `data-label`/`data-priority`), `renderers/table-cells.ts` (`renderDeltaCell`), `table-mount.ts` (`applyBreakpointClass`: explicit density wins, else content-pressure condense, cards below 400px). CSS `packages/core/src/styles/table.css`. |
...
| Chrome economy (what a small container may draw) | `packages/core/src/responsive/breakpoints.ts` → `resolveChromeEconomy(width, height)`, `HEIGHT_NO_GRID_MAX` 150, `COMPACT_MAX_X_TICKS` 3. Consumed once, in `compile.ts` (feeds `skipX`/`skipY` and `AxesDataContext.suppressGrid` / `.maxXTicks`). Explicit `axis` / `axis.grid` always wins. Facet panels are deliberately NOT run through it (the economy is a container-size rule, not a panel-size rule). |
