...
function buildPositionalScale(
  ...
  switch (channel.type) {
    ...
    case 'nominal':
    case 'ordinal':
      // Bar and range charts use band scales for their categorical axis (both orientations)
      if (
        chartType === 'bar' ||
        chartType === 'range' ||
...
export function computeScales(
  ...
  // Scatter/bubble and range charts should NOT include zero by default
  // (both encode position, not length; a tight domain fits the data range)
  if (spec.markType === 'point' || spec.markType === 'range') {
