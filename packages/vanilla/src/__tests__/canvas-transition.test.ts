...
function scatter(
  ...
  return {
    ...
    //
    // The backend is the mount-level `renderer` option (or `'auto'` point-count
    // promotion) since v8 — specs no longer carry it.
    animation: {
      enter: false,
...
describe('canvas keyed morph', () => {
  it('runs a transition for a 4,341-point keyed update', () => {
    const { chart } = mountCanvas(scatter(IDS_2019, 0));
    const handle = chart.beginManualUpdate(scatter(IDS_2025, 33));
  ...
  it('paints the rewound from-state synchronously, before the first step', () => {
    ...
    const dest = mountCanvas(scatter(IDS_2025, 33));
  ...
  it('keeps the SVG free of point circles and ghosts throughout', () => {
    ...
    const { container, chart } = mountCanvas(scatter(IDS_2019, 0));
    const handle = chart.beginManualUpdate(scatter(IDS_2025, 33));
    expect(handle).not.toBeNull();
  ...
  it('lands on exactly the geometry a fresh render of the destination produces', () => {
    ...
    const destSpec = scatter(IDS_2025, 33);
...
describe('canvas transition gating', () => {
  it('runs above the SVG cap without any maxMarks override', () => {
    const { chart } = mountCanvas(scatter(IDS_2019, 0));
    expect(chart.beginManualUpdate(scatter(IDS_2025, 33))).not.toBeNull();
    chart.destroy();
  });

  it('falls back to an instant swap past the canvas cap', () => {
    const big = range(0, 20_002);
    const { chart } = mountCanvas(scatter(big, 0));
    expect(chart.beginManualUpdate(scatter(big, 33))).toBeNull();
    chart.destroy();
  });

  it('honors an explicit maxMarks above the canvas cap', () => {
    const big = range(0, 20_002);
    const { chart } = mountCanvas(scatter(big, 0, { maxMarks: 30_000 }));
    expect(chart.beginManualUpdate(scatter(big, 33, { maxMarks: 30_000 }))).not.toBeNull();
    chart.destroy();
  });

  it('bars a canvas -> svg flip with mass exits (they would become SVG ghosts)', () => {
    // 4,341 canvas dots down to 400 SVG dots: `'auto'` demotes the destination
    // below the threshold. Judging by the destination count alone this looks
    // trivially cheap, but the ~3,941 exits render as SVG ghost circles in the
    // destination mode -- exactly the jank the cap exists to prevent. (With the
    // backend fixed at mount, auto demotion is the only way to reach this flip,
    // and it always trips the cap.)
    const container = createContainer(600, 400);
    const chart = createChart(container, scatter(IDS_2025, 0), {
      width: 600,
      height: 400,
    });
    expect(container.querySelector('canvas.oc-mark-canvas')).not.toBeNull();

    const handle = chart.beginManualUpdate(scatter(range(0, 400), 33));
  ...
  it('allows an svg -> canvas flip (exits paint on the canvas)', () => {
    const container = createContainer(600, 400);
    const chart = createChart(container, scatter(range(0, 400), 0), {
      width: 600,
      height: 400,
    });
    expect(container.querySelector('canvas.oc-mark-canvas')).toBeNull();

    const handle = chart.beginManualUpdate(scatter(range(50, 4341), 33));
    expect(handle).not.toBeNull();
    expect(container.querySelector('canvas.oc-mark-canvas')).not.toBeNull();
    chart.destroy();
  });
  ...
  it('does nothing under reduced motion', () => {
    ...
    const { chart } = mountCanvas(scatter(IDS_2019, 0));
    expect(chart.beginManualUpdate(scatter(IDS_2025, 33))).toBeNull();
    chart.destroy();
  });
});
...
describe('canvas transition interruption', () => {
  it('retargets a second update from the interrupted positions', () => {
    ...
    chart.update(scatter(IDS_2025, 33));
    pumpRaf(0);
    pumpRaf(200);
    ...
    const handle = chart.beginManualUpdate(scatter(IDS_2025, 66));
    expect(handle).not.toBeNull();
    handle!.step(5000);
    chart.destroy();
  });

  it('survives an interruption across an svg -> canvas mode flip', () => {
    // The snapshot entries are plain `{type:'point'}` in both modes, so the
    // canvas builder can consume what the SVG tween froze. (The reverse
    // direction with a live handle is unreachable now that the backend is
    // fixed at mount: an auto canvas -> svg demotion means dropping from
    // >1,000 to <1,000 points, which always exceeds the SVG update cap and
    // swaps instantly -- covered by the mass-exits test above.)
    const container = createContainer(600, 400);
    const chart = createChart(container, scatter(range(0, 400), 0), {
      width: 600,
      height: 400,
    ...
    expect(container.querySelector('canvas.oc-mark-canvas')).toBeNull();
    chart.update(scatter(range(0, 400), 33));
    pumpRaf(0);
    pumpRaf(200);

    const handle = chart.beginManualUpdate(scatter(range(0, 1200), 66));
    expect(handle).not.toBeNull();
    expect(container.querySelector('canvas.oc-mark-canvas')).not.toBeNull();
    handle!.step(5000);
    chart.destroy();
...
describe('canvas transition leaves the SVG layer working', () => {
  it('still tweens the trendline path while dots move on canvas', () => {
    const spec = (shift: number) => scatter(IDS_2019, shift, { trendline: true });
    ...
    const handle = chart.beginManualUpdate(scatter(IDS_2025, 33, { trendline: true }));
    expect(handle).not.toBeNull();
